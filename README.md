...
* **ランク別ロゴサイズ指定**: 各ランクごとにロゴの横幅や縦高を統一して表示する機能
...
* WordPress v6.3以降 (Gutenberg v15.2対応のため)
* PHP v7.4以降
* JavaScript が有効な最新の Web ブラウザ
* Node.js v18以降 (React v19.2対応のため)
* npm または yarn
...
3. 表示順序を設定し、説明を追加します。
4. **ロゴサイズの設定** (オプション):
   * **ロゴサイズ指定フィールド** で、そのランクに属するパートナーロゴの表示サイズを統一できます。
   * **サイズ指定タイプ** を選択します:
     * **サイズ指定なし**: 既存の CSS (`max-width: 100%`, `height: auto`) に従って自動的にスケーリングされます。親要素の幅に応じて、画像の元のアスペクト比を保ったまま適切にスケーリングされます。
     * **このランクの横幅を設定**: 指定したピクセル値 (例: 188px) で横幅を固定します。高さは自動的にアスペクト比を維持して調整されます。
     * **このランクの縦高を設定**: 指定したピクセル値 (例: 96px) で縦高を固定します。横幅は自動的にアスペクト比を維持して調整されます。
   * サイズ指定タイプで「横幅」または「縦高」を選択した場合、**サイズ値** フィールドが表示されます。ここにピクセル単位の数値を入力してください。
   * フロントエンドでは、同じランクに属するすべてのパートナーロゴが、指定されたサイズで統一して表示されます。
5. 変更を保存します。

* **ヒント**: ロゴサイズを指定することで、同じランク内のパートナーロゴの見た目を統一し、よりプロフェッショナルな表示が可能になります。たとえば、Silver ランクのロゴを縦高96px に統一したり、Gold ランクのロゴを横幅188px に統一したりできます。
...
* **ヒント**: パートナーロゴの表示サイズは、そのパートナーが属するランクで設定されたロゴサイズ設定に従います。ランクでロゴサイズが指定されていない場合、既存の CSS ルールに従って自動的にスケーリングされます。
...
A: Gutenberg ブロック・エディターとの最適な互換性のため、WordPress v6.3以降が必要です。フロント表示向けの `block.json` `viewScript` は WordPress v6.1以降で解釈されますが、プラグイン全体としては v6.3未満は対象外です。
...
### Q: ランクごとにロゴサイズを統一することはできますか ?

A: はい、ランクラベル設定画面で、各ランクに対してロゴの横幅または縦高を指定できます。指定したサイズは、そのランクに属するすべてのパートナーロゴに自動的に適用されます。サイズを指定しない場合、既存の CSS ルールに従って自動的にスケーリングされます。
...
interface RankLabel {
  ...
  logo_size_value?: number; // ロゴサイズの値 (width または height、ピクセル単位)
}
```
...
interface ContentModel {
  ...
  logo_size_value?: number; // ロゴサイズの値 (ランクから継承、ピクセル単位)
}
```

...
* **Vite v7.2**: 高速ビルドとモジュール・バンドリング (✅完全実装済み)
...
* **Carousel 表示機能** (**実装状況**: ⏳未実装 - 本修正で追加予定):
  * **ランク別 Carousel 表示**: 管理画面で各ランクラベルに対して Carousel 表示を指定可能
  * **表示条件**: 各ランクに属するコンテンツモデル (`frontpage='YES'`) の数が4〜8の場合のみ、Carousel を指定可能
  * **Carousel 有効時**: `carousel_enabled=true` の場合、そのランクのバナーを Carousel 形式で表示
    * スライドナビゲーション (前へ/次へボタン) を表示
    * インジケーター (ドット) を表示
    * 自動スライド機能 (オプション)
    * タッチスワイプ対応 (モバイル環境)
  * **Carousel 無効時**: `carousel_enabled=false` または未設定の場合、既存のグリッド表示を維持
  * **動的判定**: フロントエンド表示時に、各ランクのコンテンツモデル数を動的にカウントし、Carousel 表示の可否を判定
  * **フォールバック**: 子要素数が4未満または8超過の場合、Carousel 設定が有効でも通常のグリッド表示にフォールバック
...
#### 8.1.4. ランクラベル管理 (**実装状況**: ✅100% → ⏳未実装機能追加予定)
...
* 代わりとなる一覧画面を React UI で実装します。項目数が少ないので、インラインで直接編集を可能にします (項目数が増えた段階で、モーダル画面で編集する様に変更)。
  ...
  * **Carousel 表示指定フィールド** (`carousel`): (**実装状況**: ⏳未実装 - 本修正で追加予定)
...
* **Carousel 表示指定機能** (**実装状況**: ⏳未実装 - 本修正で追加予定):
    * **CheckboxControl による Carousel 表示の有効/無効切り替え**: ランク毎に Carousel 表示を指定可能
...
### 9.1. データ構造と型定義 (**実装状況**: ✅完全実装済み - 全型定義が実装済み → ⏳未実装機能追加予定)
...
* **RankLabel 型定義の詳細** (**実装状況**: ✅100% 完了 → ⏳未実装機能追加予定):
  ...
  export interface RankLabel {
    ...
    carousel_enabled?: boolean; // Carousel 表示の有効/無効 (本修正で追加)
  }
  ```
  ...
  * `carousel_enabled`: Carousel 表示の有効/無効 (本修正で追加)
...
* **データ保存**:
  ...
  * ランクラベルの Carousel 設定は、カスタム投稿タイプ `s2j_am_rank_label` のメタデータとして保存されます (本修正で追加):
...
* **データ取得**:
  ...
  * REST API エンドポイント `GET /wp-json/s2j-alliance-manager/v1/rank-labels` でランクラベル情報とともに Carousel 設定を取得 (本修正で追加)
  * フロントエンド表示時、PHP 側でコンテンツモデルを準備する際に、各ランクに対応するロゴサイズ情報を取得し、ContentModel に追加
  * フロントエンド表示時、PHP 側でコンテンツモデルを準備する際に、各ランクに対応する Carousel 設定を取得し、ランク別の表示制御に使用 (本修正で追加)

### 9.2. データフローと状態管理 (**実装状況**: ✅完全実装済み - すべての状態管理の機能が実装済み → ⏳未実装機能追加予定)
...
* **Carousel 設定の伝播**: ランクラベルで設定された Carousel 設定は、フロントエンド表示時に自動的に各ランクの表示制御に反映されます（本修正で追加）。
* **子要素数の動的管理**: コンテンツモデルの変更時に、各ランクの子要素数を再計算し、Carousel 設定の有効/無効を動的に更新します (本修正で追加)。

### 9.3. REST API 仕様 (**実装状況**: ✅完全実装済み → ⏳未実装機能追加予定)
...
* `GET /wp-json/s2j-alliance-manager/v1/rank-labels`
  ...
  * レスポンスに `carousel_enabled` を含む (本修正で追加)
* `POST /wp-json/s2j-alliance-manager/v1/rank_labels`
  * ランクラベル一括保存
  * リクエストボディに `logo_size_type` と `logo_size_value` を含む
  * リクエストボディに `carousel_enabled` を含む (本修正で追加)
  * 保存時に、子要素数が4〜8の範囲外の場合、`carousel_enabled` を `false` に強制設定 (本修正で追加)
...
* ⏳**Carousel 表示機能**: 0% 完了 - 本修正で実装予定
  * ランク別 Carousel 表示指定機能
  * 子要素数の動的カウント機能
  * 条件付き有効化機能
  * フロントエンド Carousel 表示機能

### 10.4. 実装完了率

* **全体**: 約92% 完了 → 本修正後: 約94% 完了 (予定)
* **コア機能**: 100% 完了
* **管理 UI**: 95% 完了 (主要機能は完全実装済み) → 本修正後: 97% 完了 (予定)
* **フロントエンド表示**: 100% 完了 → 本修正後: 100% 完了 (Carousel 機能追加)
* **アクセシビリティ**: 95% 完了
* **国際化**: 100% 完了
...
* **Carousel 表示機能の実装**: 本修正で追加予定 (優先度: 高)
...
* **Carousel 機能の拡張**: 自動スライド機能、カスタマイズ・オプション (優先度: 中)
...
残り8%の未実装機能は主に細かな改善項目であり、現在の実装でも十分に実用的です。
本修正により、Carousel 表示機能が追加され、より柔軟な表示制御が可能になります。
今後の段階的な改善により、さらに完璧なプラグインとなることが期待されます。

### 13.3. 推奨アクション

1. **即座に実装可能**: Carousel 表示機能の実装 (1-2日)
2. **短期間で実装可能**: MessageModal の細かな改善 (1-2時間)、SettingsForm の高度なプレビュー機能 (1-2日)
